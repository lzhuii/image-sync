...
部分源镜像（如 `gitea/gitea`）的 manifest list 包含空的占位 manifest。ACR 个人版拒绝 `application/vnd.oci.empty.v1+json` 媒体类型。本仓库使用 `skopeo copy -a`（等价于 `--all`），会复制完整多架构 manifest list，因此这类镜像会失败。可行方案：

1. 从 `images.txt` 移除该镜像，手动同步。
2. 升级 skopeo 到 1.13+（自行编译或手动安装），以启用 `--multi-arch=linux/amd64,linux/arm64` 按平台复制。
